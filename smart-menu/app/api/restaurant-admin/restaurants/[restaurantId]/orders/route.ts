...
export async function GET(req: NextRequest, context: RouteContext) {
  ...
  if (!backendBase) {
    console.error("Admin orders proxy error: BACKEND_URL missing.");
  ...
  const url = `${backendBase}/api/restaurant-admin/restaurants/${restaurantId}/orders${
    search ? `?${search}` : ""
  }`;
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, data: null, error: "Failed to fetch orders." },
      { status: 500 }
    );
  }
}
