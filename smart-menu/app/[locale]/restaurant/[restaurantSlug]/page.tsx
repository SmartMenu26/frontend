...
const DESCRIPTION_COPY: Record<Locale, (name: string, city?: string) => string> = {
  mk: (name, city) =>
    `Погледнете го менито на ${name}${city ? ` во ${city}` : ""}. QR мени, цени, алергени и препораки.`,
  sq: (name, city) =>
    `Shikoni menunë e ${name}${city ? ` në ${city}` : ""}. Menü QR, çmime, alergjenë dhe rekomandime.`,
  en: (name, city) =>
    `Browse the menu of ${name}${city ? ` in ${city}` : ""}. QR menu, prices, allergens, and AI recommendations.`,
};
