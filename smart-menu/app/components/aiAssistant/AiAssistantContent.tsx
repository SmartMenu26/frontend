  ...
  return (
    <div className="min-h-dvh bg-[#F5F5F5] text-[#1E1F24]">
      ...
      <div className="mx-auto flex w-full max-w-md flex-col px-6">
        ...
        <div className="h-[90vh] text-center flex flex-col justify-around">
          {status === "idle" && (
          ...
          {status !== "loading" && candidates.length > 0 && (
            <div className="max-h-[40vh] overflow-auto mt-4 flex flex-col gap-3 text-left">
              {assistantText && (
                <p className="mt-6 rounded-2xl bg-[#F4F5F7] px-2 text-left text-sm text-[#4B4F54]">
                  {assistantText}
                </p>
              )}
              {candidates.map((item) => {
