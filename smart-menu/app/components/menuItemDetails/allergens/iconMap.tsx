...
const MushroomIcon: LucideIcon = ({
  ...
  const resolvedSize = parseSize(size);
  const { ["aria-label"]: ariaLabel, ["aria-hidden"]: _ariaHidden, ...domRest } =
...
const ALLERGEN_ICON_MAP: Record<string, AllergenIconEntry> = {
  ...
  mushroom: {
    icon: MushroomIcon,
    defaultTooltipKey: "mushroom.contains",
  },
  mushrooms: {
    icon: MushroomIcon,
    defaultTooltipKey: "mushroom.contains",
